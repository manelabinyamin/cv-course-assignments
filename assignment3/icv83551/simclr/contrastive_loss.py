...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        pos_sim_k = torch.exp(sim(z_k, z_k_N) / tau)
        ...
        total_loss += -torch.log(pos_sim_k / denom_k)

        pos_sim_k_N = torch.exp(sim(z_k_N, z_k) / tau)
        denom_k_N = 0
        for i in range(2 * N):
            if i != (k + N):
                denom_k_N += torch.exp(sim(z_k_N, out[i]) / tau)

        total_loss += -torch.log(pos_sim_k_N / denom_k_N)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.mm(out, out.t()) / (
        torch.linalg.norm(out, dim=1).view(-1, 1)
        * torch.linalg.norm(out, dim=1).view(1, -1)
    )
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def simclr_loss_vectorized(out_left, out_right, tau, device="cuda"):
    ...
    masked_exponential = exponential * mask

    # Hint: Compute the denominator values for all augmented samples. This should be a 2N x 1 vector.
    denom = torch.sum(masked_exponential, dim=1, keepdim=True)  # [2*N, 1]

    # Step 2: Compute similarity between positive pairs.
    # You can do this in two ways:
    # Option 1: Extract the corresponding indices from sim_matrix.
    # Option 2: Use sim_positive_pairs().
    pos_pairs = sim_positive_pairs(
        torch.cat([out_left, out_right], dim=0), torch.cat([out_right, out_left], dim=0)
    )  # [2*N, D]
...
def simclr_loss_vectorized_(out_left, out_right, tau, device="cuda"):
    """Compute the contrastive loss L over a batch (vectorized version). No loops are allowed.
    ...
    # Option 1: Extract the corresponding indices from sim_matrix.
    # Option 2: Use sim_positive_pairs().
    pos_pairs = exponential.diag().view(-1, 1)  # [N]
    ...
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = (
        -(numerator / denom).log().mean()
    )  # -torch.mean(torch.log(numerator / denom))  # [1]
